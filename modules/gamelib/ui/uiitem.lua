...
function UIItem:onItemChange()
  local tooltip = nil
  local itemRarity = "commum"
  if self:getItem() and self:getItem():getTooltip():len() > 0 then
    tooltip = self:getItem():getTooltip()
  end
  if self:getItem() and self:getItem():getRarity():len() > 0 then
    itemRarity = self:getItem():getRarity()
    rarity = self:getItem()
	 self:setImageSource('/images/game/raritys/rarity_'.. itemRarity..'.png')
	 rarity:setShader("ShaderWidgetGlow")
	-- else if self:getItem() and self:getItem():getRarity():len() < 1 then
	 -- self:setImageSource('/images/game/raritys/rarity_commum.png')
	else
	 self:setImageSource('/images/ui/item.png')
    --rarity:setShader(itemRarity..'_rarity')
	end
  -- end
